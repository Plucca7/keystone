...
//
// File split to respect the tool's own 400-line guard: the spec OS rule tests (constitution,
// clarify, plan-and-tasks, verify, test-first-by-risk, subagent-driven-development, discovery)
// and the Layer C experience-quality tests used to live here too, pushing this file past the
// 400-line limit that `keystone check` enforces on every file. They now live in
// harness-rules.test.ts; this file keeps the harness scaffolding, the pre-push gate, the
// server-side rails, and the two guardrail-hook tests. No test content was changed by the split
// -- only moved.
