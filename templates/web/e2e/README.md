...
| 3. **Integration**   | Data access against a **real database** — mocks lie: they encode what you believe the database does, and pass while production fails. Runs against a disposable local PostgreSQL with the real migrations applied (`scripts/db-migrate.sh`); skips cleanly via `describe.skipIf` when `DATABASE_URL` is unset, so `pnpm test` stays green with no database wired in | Vitest     | `src/__tests__/integration/tenant-isolation.test.ts`  |
