...
//
// Overrides the project-wide jsdom environment (vitest.config.ts) for this file only: PGlite's
// WASM/filesystem bootstrap calls a raw Response's `.arrayBuffer()` during startup, which jsdom's
// fetch/Response polyfill does not provide. The Node environment has it.
/**
...
import { withTransaction, type TransactionalConnection } from '../../lib/transaction'
