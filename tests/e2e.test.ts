...
test('e2e: a freshly created system project is born in the standard', async () => {
  ...
  try {
    const answers: KeystoneAnswers = {
      product: { name: 'shop', type: 'system', language: 'pt', screen: 'both', look: 'later', sensitive: true },
...
test('e2e (the test dummy): analysis catches every planted flaw', async () => {
  ...
  try {
    ...
    assert.ok(failedTitles.some((t) => t.includes('conventions')), 'catches the bad database');
    assert.ok(failedTitles.some((t) => t.includes('oversized')), 'catches the oversized file');
  } finally {
