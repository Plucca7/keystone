...
example `feat(items): add archive endpoint`). Commit messages are linted by
commitlint (`config/commitlint-config/`) and enforced by a Git hook — an
unscoped or wrongly-typed commit is rejected before it lands.
...
pnpm lint
pnpm test
pnpm run test:coverage   # thresholds enforced, see vitest.config.ts
pnpm build
```
...
a failing test that reproduces the bug. See [CLAUDE.md](CLAUDE.md) and
[e2e/README.md](e2e/README.md) for the four-layer test pyramid this project
follows.
